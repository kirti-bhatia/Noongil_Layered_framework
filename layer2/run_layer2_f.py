...
def choose_scenario_interactively() -> tuple[Optional[str], bool]:
    """Display discovered scenarios and ask the user what to run.

    Returns
    -------
    (scenario_name, run_all)
        ``scenario_name`` contains the selected scenario. ``run_all`` is
        True when the user chooses to process every discovered scenario.
    """
    ...
    if not scenarios:
        raise Layer2PipelineError(
            "No compatible Layer 1 scenarios were discovered. "
            "Each pipeline scenario must contain a valid "
            "layer1_sensor_packet.json and its referenced media files."
        )

    print("=" * 72)
    print("NOONGIL-X | SELECT A LAYER 2 TEST SCENARIO")
    print("=" * 72)
    ...
    for index, scenario in enumerate(
        ...
        print(f"  {index:>2}. {scenario}")

    print(f"  {len(scenarios) + 1:>2}. Run all scenarios")
    print("   Q. Quit")

    while True:
        answer = input(
            "\nEnter scenario number or name: "
        ).strip()
        ...
        if answer.isdigit():
            ...
            if 1 <= selection <= len(scenarios):
                return scenarios[selection - 1], False

            if selection == len(scenarios) + 1:
                return None, True

        normalized_answer = answer.lower()

        for scenario in scenarios:
            if scenario.lower() == normalized_answer:
                return scenario, False

        print(
            "Invalid choice. Enter a displayed number, "
            "an exact scenario name, A, or Q."
        )
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Scenario to process. If omitted, an interactive "
...
def main() -> int:
    ...
    run_every_scenario = arguments.all

    if not run_every_scenario and selected_scenario is None:
        try:
            selected_scenario, run_every_scenario = (
                choose_scenario_interactively()
            )
        except Layer2PipelineError as error:
            print(f"[FAILED] {error}")
            return 1

        if selected_scenario is None and not run_every_scenario:
            print("Layer 2 test cancelled.")
            return 0

    if run_every_scenario:
